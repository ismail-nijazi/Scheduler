...
function TimelineTask({ task, startOfWeek }) {
	...
	const getTaskStyle = () => {
		...
		let startToEndHours = (Math.abs(
			startOfWeek - taskDeadline) / 36e5) / COUNT_OF_HOURS_IN_WEEK;
		
		if (taskStartTime >= startOfWeek) {
			startToEndHours = (Math.abs(
			taskStartTime - taskDeadline) / 36e5) / COUNT_OF_HOURS_IN_WEEK;
			startLeft = (Math.abs(
			taskStartTime - startOfWeek) / 36e5) / COUNT_OF_HOURS_IN_WEEK;
		}
