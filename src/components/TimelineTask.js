...
function TimelineTask({ task, startOfWeek, endOfWeek }) {
  ...
  const diff = {
    deadlineToStart: Math.floor(Math.abs(task.deadline - task.starting_time) / 3600000),
    endOfWeekToStart: Math.floor(Math.abs(endOfWeek - task.starting_time) / 3600000),
    startOfWeekToDeadline: Math.floor(Math.abs(startOfWeek - task.deadline) / 3600000)
  };
  ...
  const getTaskStyleWeek = () => {
    ...
    let startLeft = 0;
    let startToEndHours =
      Math.abs(startOfWeek - taskDeadline) / 36e5 / COUNT_OF_HOURS_IN_WEEK;

    if (taskStartTime >= startOfWeek) {
      startToEndHours =
        Math.abs(taskStartTime - taskDeadline) / 36e5 / COUNT_OF_HOURS_IN_WEEK;
      startLeft = Math.abs(taskStartTime - startOfWeek) / 36e5 / COUNT_OF_HOURS_IN_WEEK;
    }
    return {
